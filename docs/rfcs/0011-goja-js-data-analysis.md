...
## 摘要
本 RFC 在 `xsql ai` 中集成纯 Go 实现的 `goja` JavaScript 虚拟机（100% Zero CGO），构建 **Session DataStore（会话数据集存储与召回）** 机制，并实现无硬编码的 **ReAct Tool Agent Loop 循环推理**。

## 背景 / 动机
- AI TUI 原本仅支持 SQL 交互与表格展示，缺少数据二次聚合计算、跨查询结果 Join/比对以及结构化导出文件（CSV/JSON/Markdown）的能力。
...
- 支持 Context 超时打断（默认 1 分钟），并自动捕获 `console.log` 输出。
...
- 向大模型上下文提供轻量 **Dataset Catalog** 元数据目录；本地 JS 的派生结果仅以有长度上限的摘要回传，原始查询结果不会自动加入模型上下文。
...
所有 Tool Calls 默认在 TUI 容器中折叠内嵌呈现（`Ctrl+O` 展开/折叠，`Tab` 切换焦点），且交互末尾必定以 LLM 自然语言 Markdown 分析报告总结收尾。
